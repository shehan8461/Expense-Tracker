...
const handledelete= async(id)=>{
    const data=await axios.delete("http://localhost:8080/delete_expense/"+id)
    if(data.data.success){
        getfetchdata()
...
const filterdata = (searchKey) => {
    const filteredData = showcustomer.filter(customer =>
        customer.type.toLowerCase().includes(searchKey.toLowerCase())
    );
 ...
 <table>     
              ...
              <tbody>
                  { 
                     showcustomer.map((e1)=>{
                      return(
                          <tr> 
                            <td> {e1.expense_amount}</td> 
                            <td> {e1.expense_type}</td> 
                            <td> {e1.expense_bank_type}</td> 
                            <td> {e1.expense_remark}</td> 
                            
                            <td className='dback'>
                              <a href={`/update_expense/${e1._id}`}>Edit Details</a>
  ...
  <br></br>
  <button onClick={generatePDF}>Download Expense Repoart</button>
        </div>
