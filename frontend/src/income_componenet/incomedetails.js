...
const handledelete= async(id)=>{
    const data=await axios.delete("http://localhost:8080/delete_income/"+id)
    if(data.data.success){
        getfetchdata()
              ...
              <tbody>
                  { 
                     showcustomer.map((e1)=>{
                      return(
                          <tr> 
                            <td> {e1.income_amount}</td> 
                            <td> {e1.income_type}</td> 
                            <td> {e1.income_bank_type}</td> 
                            <td> {e1.income_remark}</td> 
                            
                            <td className='dback'>
                              <a href={`/update_income/${e1._id}`}>Edit Details</a>
